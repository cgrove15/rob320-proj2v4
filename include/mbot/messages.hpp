...
#define ROS_PKG_LENGTH \
    (ROS_HEADER_LENGTH + ROS_FOOTER_LENGTH) /**< Length of the ROS packet overhead */
...
static inline int encode_msg(uint8_t* msg, int msg_len, uint16_t topic, uint8_t* rospkt,
                             int rospkt_len) {
    // SANITY CHECKS
...
static inline int mbot_motor_vel_t_deserialize(const uint8_t* src,
                                               serial_mbot_motor_vel_t* dest);
...
typedef struct __attribute__((__packed__)) serial_mbot_message_received_t {
    ...
    int64_t creation_time;  // time of message creation (assumption that this will be
                            // unique between messages)
    char channel[256];      // name of channel
} serial_mbot_message_received_t;

static inline int mbot_message_received_t_deserialize(
    const uint8_t* src, serial_mbot_message_received_t* dest);
static inline int mbot_message_received_t_serialize(serial_mbot_message_received_t* src,
                                                    uint8_t* dest);
...
static inline int slam_status_t_deserialize(const uint8_t* src,
                                            serial_slam_status_t* dest);
...
static inline int mbot_motor_pwm_t_deserialize(const uint8_t* src,
                                               serial_mbot_motor_pwm_t* dest);
...
typedef struct __attribute__((__packed__)) serial_particle_t {
    ...
    serial_pose2D_t
        parent_pose;  // (x,y,theta) of the prior pose the new estimate came from
...
static inline int planner_request_t_deserialize(const uint8_t* src,
                                                serial_planner_request_t* dest);
static inline int planner_request_t_serialize(serial_planner_request_t* src,
                                              uint8_t* dest);
...
static inline int mbot_encoders_t_deserialize(const uint8_t* src,
                                              serial_mbot_encoders_t* dest);
...
static inline int exploration_status_t_deserialize(const uint8_t* src,
                                                   serial_exploration_status_t* dest);
static inline int exploration_status_t_serialize(serial_exploration_status_t* src,
                                                 uint8_t* dest);
...
typedef struct __attribute__((__packed__)) serial_mbot_slam_reset_t {
    int64_t utime;
    int32_t slam_mode;  // mapping_only=0, action_only=1, localization_only=2, full_slam=3
    char slam_map_location[256];  // only necessary when for localization-only and
                                  // action_only modes
    bool retain_pose;             // Whether to keep the pose when resetting.
} serial_mbot_slam_reset_t;

static inline int mbot_slam_reset_t_deserialize(const uint8_t* src,
                                                serial_mbot_slam_reset_t* dest);
static inline int mbot_slam_reset_t_serialize(serial_mbot_slam_reset_t* src,
                                              uint8_t* dest);
...
int mbot_message_received_t_deserialize(const uint8_t* src,
                                        serial_mbot_message_received_t* dest) {
    memcpy(dest, src, sizeof(serial_mbot_message_received_t));
    return 1;
}

int mbot_message_received_t_serialize(serial_mbot_message_received_t* src,
                                      uint8_t* dest) {
    memcpy(dest, src, sizeof(serial_mbot_message_received_t));
    return 1;
}
...
int exploration_status_t_deserialize(const uint8_t* src,
                                     serial_exploration_status_t* dest) {
