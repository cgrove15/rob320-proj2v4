...
class MockClient : public rix::ipc::interfaces::Client {
   public:
    MockClient(size_t capacity, const std::string &address, std::shared_ptr<std::map<rix::ipc::Endpoint, MockServer*>> server_map)
        : server_map(server_map), is_connecting(false), connected(false) {
        ...
        ON_CALL(*this, connect).WillByDefault([this](const rix::ipc::Endpoint &endpoint) -> bool {
            ...
            auto it = this->server_map->find(endpoint);
            if (it == this->server_map->end()) return false;
            server = it->second;
        ...
        ON_CALL(*this, reset).WillByDefault([this, capacity](){
            endpoints = std::make_shared<std::array<rix::ipc::Endpoint, 2>>();
   ...
   private:
    ...
    bool is_connecting;
    bool connected;
    std::shared_ptr<std::map<rix::ipc::Endpoint, MockServer*>> server_map;
    MockServer *server;
